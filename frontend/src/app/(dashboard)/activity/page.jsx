...
const Page = () =>
 {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-800 font-sans">
      {/* SIDEBAR NAVIGATION */}
      <aside className="w-64 border-r border-slate-200 bg-white p-6 flex flex-col gap-6 shrink-0">
        <div className="text-xl tracking-wide font-bold border border-slate-200 rounded-full py-2 px-4 text-center text-indigo-600 mb-4 shadow-sm bg-slate-50">
          VendorBridge
        </div>
        <nav className="flex flex-col gap-1">
          {["Dashboard", "Vendors", "RFQ's", "Quotations", "Approvals", "Purchase orders", "Invoices", "Reports"].map((navItem) => (
            <Link key={navItem} href="#" className="py-3 px-4 text-slate-500 hover:text-indigo-600 font-medium transition-colors">
              - {navItem}
            </Link>
          ))}
          {/* Active State */}
          <Link href="#" className="py-3 px-4 text-indigo-700 border border-indigo-200 bg-indigo-50 rounded-lg font-bold shadow-sm">
            - Activity
          </Link>
        </nav>
      </aside>
      ...
      {/* MAIN WORKSPACE */}
      <main className="flex-1 p-10 max-w-6xl grid grid-cols-1 xl:grid-cols-3 gap-12 bg-slate-50">
        
        {/* Left/Center Column: Activity Feed */}
        <div className="xl:col-span-2">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl text-slate-900 font-bold mb-2 tracking-tight">
              Activity & Logs
            </h1>
          ...
          <div className="space-y-3">
            {filteredLogs.length > 0 ? (
              filteredLogs.map((log) => (
                <div key={log.id} className="flex gap-5 items-center border border-slate-200 bg-white py-5 px-6 rounded-xl shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="py-12 text-center text-slate-500 bg-white border border-dashed border-slate-300 rounded-xl">
        ...
        {/* Right Column: Security/Admin Notice */}
        <div className="hidden xl:block">
          <div className="border border-indigo-100 bg-indigo-50/50 rounded-xl p-6 sticky top-10 shadow-sm">
